...
import { withTimeout, ProofTimeoutError, DEFAULT_PROOF_TIMEOUT_MS } from "@/lib/proof-timeout";
import { proveOffMainThread } from "@/lib/proof-client";
...
// Stages the prover worker reports ("witness" → "circuit" → "proof") plus
// this component's own UI states. "proving" is gone: the worker's own
// progress messages now drive the sub-steps directly.
type Stage = "witness" | "circuit" | "proof" | "generated" | "submitting" | "confirmed" | "error";

/** Stages in which a proof is still being generated (progress UI is live). */
const isGenerating = (stage: Stage): boolean =>
  stage === "witness" || stage === "circuit" || stage === "proof";
...
function ProofFlow({
  ...
  const startElapsedTimer = (from: number) => {
  ...
  // fetch cancelled, WASM backend destroyed) and returns to the list.
  const onCancelProve = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Witness generation and UltraHonk proving both run in the dedicated
        // prover worker; only progress messages come back, so the UI thread
        // stays free to repaint the progress bar and accept a cancel click.
            ...
            proveOffMainThread(
              ...
              },
              {
                ...
                onProgress: (stage) => {
                  ...
                  if (stage !== "witness" && !provingStarted) {
                    provingStarted = true;
                    startElapsedTimer(Date.now());
                  }
                  setStage(stage);
...
function BatchProofFlow({
  ...
  // Generate proofs for all credentials in sequence. Each one runs inside the
  // dedicated prover worker, so the per-credential rows keep animating while
  // proving happens off the UI thread.
  useEffect(() => {
    const controller = new AbortController();
    abortRef.current = controller;
    const { signal } = controller;
